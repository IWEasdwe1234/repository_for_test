ln1 = f"\n{"=" * 20}\n"
ln2 = f"\n{"= " * 10}\n"
# 튜플
# 소괄호 안에 값을 쉼표로 구분해 묶으면 튜플이 만들어진다
# 튜플은 값을 바꿀 수 없다
# 리스트처럼 append·remove·sort 같은 수정용 메서드가 아예 없다
# 튜플이 가진 메서드는 딱 두 개, count와 index뿐
# 내장 함수 len을 더한 셋이 튜플을 다루는 도구 전부
#  "몇 개인가 · 몇 번 나왔나 · 어디에 있나"라는 세 가지 질문을 모두 답할 수 있다.

print(ln1)
(78)  # 그냥 숫자
(78,)  # 튜플
print("\n==[ 튜플 ]==\n")
status = ("정상", "정상", "경고", "정상")
print(f"{len(status)} ← len\n")
print(f"{status.count("정상")} ← count\n")
print(f"{status.index("경고")} ← index\n")

print(ln2)
# 튜플 리스트, 언패킹
# 리스트의 각 요소가 튜플인 구조
튜플in리스트 = [
    ("값1, 1"),
    ("값2, 2"),
    ("값3, 3"),
    ("값4, 4"),
]
print("--[ 리스트 안의 튜플 ]--\n")
print(f"{len(튜플in리스트)}\n")
print(f"{튜플in리스트[0]}\n")
print(f"{튜플in리스트[0][1]}\n")
# 튜플에서 마지막값 뒤에 쉼표를 넣어도 에러가 발생하지 않음

print(ln1)
print("--[ 튜플 리스트 정렬 ]--\n")
# 튜플 리스트 정렬
# sorted
sensors = [(78, "모터온도"), (95, "베어링진동"), (32, "펌프압력")]
a = sorted(sensors)  # (작은 값 → 큰값)
b = sorted(sensors, reverse=False)  # (작은 값 → 큰값)
c = sorted(sensors, reverse=True)  # (큰값 → 작은 값)
print(f"sorted(sensors)\n{a}")
print(f"sorted(sensors, reverse=False)\n{b}")
print(f"sorted(sensors, reverse=True)\n{c}")
# a와 b의 결과값이 같음(작은 값 → 큰값)

print(ln2)
print(f"가장 큰 값\n{c[0]}")

print(ln2)
print("--[ 튜플 언패킹 ]--\n")

리스트 = [
    ("이름1", 30),
    ("이름2", 20),
    ("이름3", 40),
    ("이름4", 10),
]
for 이름, 값 in 리스트:
    기준값 = 30
    if 값 > 기준값:
        print(이름, "기준값 초과")

print(ln1)
print("==[ 셋 ]==\n")
# set
# 중괄호 { }로 감싼 값의 묶음
# 리스트를 셋으로 감싸면 중복 제거
# 방법 1/
셋1 = {"값1", "값2", "값2", "값1", "값1", "값2", "값1"}
print("셋1 :", 셋1)  # {'값1', '값2'}

# 방법 2/ (실무에서 훨씬 잦음)
리스트 = ["값1", "값2", "값2", "값1", "값1", "값2", "값1"]
셋2 = set(리스트)
print("셋2 :", 셋2)  # {'값1', '값2'}

print(ln1)
print("--[ 빈 셋 만들기 ]--\n")
# 빈 중괄호 {}는 셋이 아니라 딕셔너리
#  빈 셋은 반드시 set()으로 만들기

empty_list = []
print(type(empty_list))  # <class 'list'> 리스트

empty_tuple = ()
print(type(empty_tuple))  # <class 'tuple'> 튜플

empty_set = {}
print(type(empty_set))  # <class 'dict'> 딕셔너리

real_empty_set = set()
print(type(real_empty_set))  # <class 'set'> 셋

print(ln1)
print("--[ .add ]--\n")
A = {"a1", "a2", "a3", "a4"}
print(A)
A.add("a3")  # 이미 존재하는 값 무시
A.add("a5")
print(A)

print(ln1)
print("--[ in ]--\n")
# 값이 셋(또는 리스트, 튜플, 문자열)안에 있으면 True, 없으면 False

A = {"a1", "a2", "a3", "a4"}
print("a3" in A)  # True
print("A3" in A)  # False
print("A5" in A)  # False

# 활용 예
if "a4" in A:
    print("a4 존재")

print(ln1)
print("--[ sorted ]--\n")
# 셋을 정렬 (결과 : 정렬된 리스트)

A = {"a2", "a4", "a1", "a3"}
st_a = sorted(A)
print(st_a)  # ['a1', 'a2', 'a3', 'a4'] ()
print(type(st_a))

# 중복 제거
B = {"a2", "a2", "a1", "a3"}  # a2가 2개
print(sorted(set(B)))  # ['a1', 'a2', 'a3'] (중복된 a2 제거, 정렬)

print(ln1)
print("--[ .union ]--\n")
# 두 셋을 합쳐 중복없는 전체 목록으로 만들기
A = {"a1", "a3", "a4", "a6"}
B = {"a5", "a2", "a4", "a1"}
print(sorted(A.union(B)))
print(sorted(B.union(A)))
print(sorted(A | B))

# 결과 : ['a1', 'a2', 'a3', 'a4', 'a5', 'a6']

print(ln1)
print("--[ .intersection ]--\n")
# 두 셋에 공통으로 들어있는 값만 남기기
print(sorted(A.intersection(B)))
print(sorted(B.intersection(A)))
print(sorted(A & B))

# 결과 : ['a1', 'a4']

print(ln1)
print("--[ .difference ]--\n")
# 앞 셋에는 있고 뒤 셋에는 없는 값만 남기기
print(sorted(A.difference(B)))  # ['a3', 'a6']
print(sorted(B.difference(A)))  # ['a2', 'a5']
print(sorted(A - B))  # ['a3', 'a6']

print(ln1)
# 딕셔너리
# 중괄호 안에 키: 값 쌍을 쉼표로 나열
# 형태 : {"키A":a,"키B":b,"키C":c}

딕셔너리 = {"키1": 1, "키2": 2, "키3": 3}
print(딕셔너리["키2"])  # 2 (값 꺼내기)

print(ln1)
print("--[ .get ]--\n")
# 만약 키가 없으면 None을 돌려줌
print(딕셔너리.get("키1"))  # 1
print(딕셔너리.get("키4"))  # None

print(ln1)
print("--[ .in ]--\n")
# .in
# 키가 있으면 True, 없으면 False
# 형태 : 키 in 딕셔너리
print("키1" in 딕셔너리)  # True
print("키4" in 딕셔너리)  # False
print(1 in 딕셔너리)  # False
print(1 in 딕셔너리.values())  # True (키 값이 있는지 확인하려면 .values()를 사용)

print(ln1)
print("--[ 딕셔너리 추가, 수정 ]--\n")
딕셔너리 = {}  # 빈 딕셔너리 시작
print(딕셔너리)
딕셔너리 = {"키1": 1}  # "키1": 1 추가
print(딕셔너리)
딕셔너리["키2"] = 2  # "키2": 2 추가
print(딕셔너리)
딕셔너리["키2"] = 3  # 키2 값 수정
print(딕셔너리)

print(ln1)
print("--[ 딕셔너리 삭제 ]--\n")
# del
print(딕셔너리)  # {'키1': 1, '키2': 3}
del 딕셔너리["키2"]
print(딕셔너리)  # {'키1': 1}

print(ln1)
print("--[ keys() ]--\n")
# 형태 : 딕셔너리.keys()
# 키만 모아서 꺼내기
딕셔너리 = {"키1": 1, "키2": 2, "키3": 3}
print(딕셔너리)  # {'키1': 1, '키2': 2, '키3': 3}
print(딕셔너리.keys())  # dict_keys(['키1', '키2', '키3'])

print(ln1)
print("--[ values() ]--\n")
# 형태 : 딕셔너리.values()
# 모든 값만 모아서 꺼내기
print(딕셔너리.values())  # dict_values([1, 2, 3])

# sum, len을 사용하여 합계·평균을 계산 가능
딕셔너리_합계 = sum(딕셔너리.values())  # 6 (합계)
딕셔너리_값갯수 = len(딕셔너리.values())  # 3 (값의 갯수)
print(딕셔너리_합계 / 딕셔너리_값갯수)  # 2.0 (값의 평균)

print(ln1)
print("--[ items() ]--\n")
# 형태 :
"""
for 키변수, 값변수 in 딕셔너리.items():
    ...
"""
# 키와 값을 짝으로 꺼내기
딕셔너리 = {"A": 1, "B": 2, "C": 3}
print(딕셔너리)

for 변수_키, 변수_값 in 딕셔너리.items():
    print(변수_키)  # A B C

for 변수_키, 변수_값 in 딕셔너리.items():
    print(변수_값)  # 1 2 3

print(ln2)
print(변수_키)  # C
print(변수_값)  # 3
# for문에서 마지막으로 할당되었던 값 "C"와 3이 출력

print(ln2)
print("[딕셔너리 대조]\n")
# 두 딕셔너리 대조 (같은 키끼리 비교)
# 형태
"""
for 키변수, 값변수 in 딕셔너리_A.items():
    if 값변수 > 딕셔너리B.get(키변수, 값변수):
        ...
"""
# 한쪽을 items()로 순회하면서, 같은 키를 다른 딕셔너리에서 꺼내 비교

딕셔너리_A = {"AA": 95, "BB": 78, "CC": 45}  # 실제 측정 데이터
딕셔너리_B = {"AA": 70, "BB": 90}  # 임계치 데이터 - 같은 키를 곻유
for 키변수, 값변수 in 딕셔너리_A.items():
    print(f"{키변수}: {값변수}")
    if 값변수 > 딕셔너리_B.get(키변수, 0):
        print("딕셔너리_A > 딕셔너리_B\n")
    else:
        print("딕셔너리_B > 딕셔너리_A\n")

print(ln2)
print("[조건에 맞는 키 모으기]\n")
# items + 조건문 + append
# 빈 리스트에 items()로 순회하면서 조건에 만족하는 키를 모으기

딕셔너리_A = {"AA": 95, "BB": 88, "CC": 4.5, "DD": 42}
딕셔너리_B = {"AA": 90, "BB": 90, "CC": 5.0, "DD": 50}

리스트 = []

for 키변수, 값변수 in 딕셔너리_A.items():
    if 값변수 > 딕셔너리_B[키변수]:
        리스트.append(키변수)

print("딕셔너리_A > 딕셔너리_B:", 리스트)
print("딕셔너리_A <= 딕셔너리_B:", len(딕셔너리_A) - len(리스트))

print(ln1)
print("--[ update() ]--\n")
# 다른 딕셔너리의 키-값을 통째로 넣기
# 형태
"""
딕셔너리_A.update(딕셔너리_B)
"""
딕셔너리_A = {"A": "a", "B": 220.4, "C": 15.1}
딕셔너리_B = {"A": "a", "D": 12.4, "E": 46.2}

딕셔너리_A.update(딕셔너리_B)
print(딕셔너리_A)
# 결과 : {'A': 'a', 'B': 220.4, 'C': 15.1, 'D': 12.4, 'E': 46.2}

print(ln1)
